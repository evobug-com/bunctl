use bunctl_core::{AppConfig, AppId, AppState, BackoffStrategy, Config};
...
async fn test_graceful_shutdown() {
    ...
    let config = AppConfig {
        ...
        command: if cfg!(windows) { "cmd".to_string() } else { "sh".to_string() },
        args: if cfg!(windows) {
            vec!["/C".to_string(), "timeout".to_string(), "/T".to_string(), "10".to_string()]
        } else {
    ...
    let _status = supervisor.graceful_stop(&mut handle, config.stop_timeout).await.unwrap();
