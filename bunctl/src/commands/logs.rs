...
async fn execute_watch_mode(args: LogsArgs) -> anyhow::Result<()> {
    ...
    match client.recv().await? {
        ...
        IpcResponse::Error { message } => {
            return Err(anyhow::anyhow!("Failed to subscribe to log events: {}", message));
        }
        _ => {
...
fn display_logs(data: &serde_json::Value, args: &LogsArgs) -> anyhow::Result<()> {
    ...
    let log_type = data.get("type").and_then(|t| t.as_str()).unwrap_or("unknown");
    
    match log_type {
        "single" => {
            let app_name = data.get("app").and_then(|a| a.as_str()).unwrap_or("unknown");
            let logs = data.get("logs").ok_or_else(|| anyhow::anyhow!("No logs data"))?;
            
            display_single_app_logs(app_name, logs, args)?;
        }
        "all" => {
            let apps = data.get("apps").and_then(|a| a.as_array())
...
fn display_single_app_logs(app_name: &str, logs: &serde_json::Value, args: &LogsArgs) -> anyhow::Result<()> {
    let empty_vec = vec![];
    let errors = logs.get("errors").and_then(|e| e.as_array()).unwrap_or(&empty_vec);
    let output = logs.get("output").and_then(|o| o.as_array()).unwrap_or(&empty_vec);
...
fn display_all_apps_logs(apps: &[serde_json::Value], args: &LogsArgs) -> anyhow::Result<()> {
    ...
    for (i, app_data) in apps.iter().enumerate() {
        ...
        if let (Some(app_name), Some(logs)) = (
            app_data.get(0).and_then(|n| n.as_str()),
            app_data.get(1)
        ) {
            display_single_app_logs(app_name, logs, args)?;
        }
