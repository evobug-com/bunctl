...
pub async fn execute(args: StartArgs) -> anyhow::Result<()> {
    ...
    let name = args.name.ok_or_else(|| anyhow::anyhow!("App name is required for ad-hoc start"))?;
    ...
    let command = if let Some(cmd) = args.command {
        ...
        return Err(anyhow::anyhow!("Either --command or --script must be provided"));
    };
