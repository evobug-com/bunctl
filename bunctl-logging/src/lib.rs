...
impl LogManager {
    pub fn new(config: LogConfig) -> Self {
        debug!(
            "Creating LogManager with base_dir: {:?}, max_file_size: {}, max_files: {}",
    ...
    pub async fn get_writer(&self, app_id: &AppId) -> Result<Arc<AsyncLogWriter>> {
        ...
        let writer = Arc::new(AsyncLogWriter::new(writer_config).await.map_err(|e| {
            error!(
                "Failed to create log writer for app {} at {:?}: {}",
                app_id, log_path, e
            );
            e
        })?);
        ...
        debug!(
            "Log writer created successfully for app: {} (total writers: {})",
            app_id,
            self.writers.len()
        );
        Ok(writer)
    ...
    pub async fn read_logs(&self, app_id: &AppId, lines: usize) -> Result<Vec<String>> {
        ...
        if all_lines.is_empty() {
            return Ok(vec![format!(
                "Log file for '{}' exists but contains no readable lines",
                app_id
            )]);
        }
    ...
    pub async fn read_structured_logs(
        &self,
        app_id: &AppId,
        lines: usize,
        ...
        if all_lines.is_empty() {
            return Ok(StructuredLogs {
                errors: vec![format!(
                    "Log file for '{}' contains no readable lines",
                    app_id
                )],
                output: vec![],
            });
