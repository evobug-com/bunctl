...
async fn test_rotation_basic() {
    ...
    assert!(!all_files.is_empty(), "Should have at least one file after rotation. Files: {:?}", all_files);
    
    // Check if original file exists
    let original_exists = all_files.iter().any(|f| f == "test.log");
    let rotated_exists = all_files.iter().any(|f| f.starts_with("test.") && f.ends_with(".log") && f != "test.log");
    
    if !rotated_exists && original_exists {
        // Rotation failed - original file still exists
        panic!("Rotation appears to have failed. Files present: {:?}", all_files);
    }
    
    assert!(rotated_exists || !original_exists, 
            "Either rotated file should exist or original should be gone. Files: {:?}", all_files);
...
async fn test_rotation_max_files_cleanup() {
    ...
    assert!(entries.len() <= 2, "Found {} files, expected <= 2", entries.len());
}
