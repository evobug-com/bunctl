...
use super::{Config, AppConfig, EcosystemConfig};
...
impl ConfigLoader {
    ...
    async fn load_package_json(&self, path: &Path) -> crate::Result<Config> {
        ...
        if let Some(bunctl_section) = package.get("bunctl") {
            let config: Config = serde_json::from_value(bunctl_section.clone())
                .map_err(|e| crate::Error::Config(format!("Failed to parse bunctl section: {}", e)))?;
            return Ok(config);
        }
        ...
        Err(crate::Error::Config("No valid config found in package.json".to_string()))
    }
    ...
    pub async fn load_file(&self, path: &Path) -> crate::Result<Config> {
        ...
        match (extension, filename) {
            ("json", _) if filename.starts_with("bunctl") => {
                self.load_bunctl_json(path).await
            }
            ...
            ("json", "package.json") => {
                self.load_package_json(path).await
            }
            _ => {
