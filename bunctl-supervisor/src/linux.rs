...
use bunctl_core::{
    AppConfig, AppId, Error, ExitStatus, ProcessHandle, ProcessInfo, ProcessSupervisor,
...
impl LinuxSupervisor {
    ...
    async fn spawn_with_cgroup(&self, config: &AppConfig) -> Result<ProcessHandle> {
        ...
        self.registry.register(app_id.clone(), ProcessHandle {
            pid,
            app_id: app_id.clone(),
            inner: None,
        });
        
        let _ = self.event_tx.send(SupervisorEvent::ProcessStarted {
            app: app_id,
            pid,
        }).await;
    ...
    fn read_proc_stat(&self, pid: u32) -> Result<HashMap<String, u64>> {
        ...
        let content = fs::read_to_string(&stat_path)
            .map_err(|_| Error::ProcessNotFound(pid.to_string()))?;
        
        let fields: Vec<&str> = content.split_whitespace().collect();
        if fields.len() < 24 {
            return Err(Error::Supervisor("Invalid /proc/pid/stat format".to_string()));
        }
