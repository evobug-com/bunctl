...
#[tokio::test]
async fn test_macos_supervisor_process_group_creation() {
...
#[tokio::test]
async fn test_macos_supervisor_kill_process_group() {
    // Test that kill_tree properly terminates all processes in the group
    let supervisor = create_supervisor().await.unwrap();
...
#[tokio::test]
async fn test_macos_supervisor_signal_handling() {
...
#[tokio::test]
async fn test_macos_supervisor_kqueue_monitoring() {
    // Test that kqueue-based monitoring works (if implemented)
    let supervisor = create_supervisor().await.unwrap();

    let config = AppConfig {
        name: "kqueue-test".to_string(),
        command: "sh".to_string(),
        args: vec![
            "-c".to_string(),
            "echo 'kqueue test' && sleep 0.1".to_string(),
        ],
        cwd: PathBuf::from("."),
        ..Default::default()
    };
...
#[tokio::test]
async fn test_macos_supervisor_env_variables() {
...
#[tokio::test]
async fn test_macos_supervisor_working_directory() {
...
#[tokio::test]
async fn test_macos_supervisor_uid_gid() {
    ...
    let config = AppConfig {
        ...
        args: vec![],
        cwd: PathBuf::from("."),
        uid: Some(501), // Common macOS user UID
        gid: Some(20),  // Common macOS staff GID
        ..Default::default()
    };

    // This will likely fail without proper permissions
...
#[tokio::test]
async fn test_macos_supervisor_multiple_processes() {
...
#[tokio::test]
async fn test_macos_supervisor_applescript_execution() {
    // Test execution of AppleScript (macOS specific)
    let supervisor = create_supervisor().await.unwrap();

    let config = AppConfig {
        name: "applescript-test".to_string(),
        command: "osascript".to_string(),
        args: vec!["-e".to_string(), "return \"AppleScript test\"".to_string()],
        cwd: PathBuf::from("."),
        ..Default::default()
    };
...
#[tokio::test]
async fn test_macos_supervisor_launchctl_interaction() {
    // Test interaction with launchctl (may require permissions)
    let supervisor = create_supervisor().await.unwrap();

    let config = AppConfig {
        name: "launchctl-test".to_string(),
    ...
    let mut handle = supervisor.spawn(&config).await.unwrap();
    let status = supervisor.wait(&mut handle).await.unwrap();
    // launchctl list should complete (success depends on context)
    assert!(status.code().is_some());
}

#[tokio::test]
async fn test_macos_supervisor_script_execution() {
...
#[tokio::test]
async fn test_macos_supervisor_resource_limits() {
    // Test resource limit setting (may be limited on macOS)
    let supervisor = create_supervisor().await.unwrap();

    let config = AppConfig {
        name: "resource-test".to_string(),
        command: "sh".to_string(),
        args: vec!["-c".to_string(), "echo 'Resource test'".to_string()],
        cwd: PathBuf::from("."),
    ...
    // Try to set resource limits (may be no-op on macOS)
...
#[tokio::test]
async fn test_macos_supervisor_exit_codes() {
...
#[tokio::test]
async fn test_macos_supervisor_concurrent_spawns() {
...
#[tokio::test]
async fn test_macos_supervisor_stress_spawn_kill() {
...
#[tokio::test]
async fn test_macos_supervisor_invalid_commands() {
...
#[tokio::test]
async fn test_macos_supervisor_sysctl_process_info() {
    // Test getting process information via sysctl
    let supervisor = create_supervisor().await.unwrap();
...
#[tokio::test]
async fn test_macos_supervisor_bundle_execution() {
    // Test execution of macOS app bundles (if available)
    let supervisor = create_supervisor().await.unwrap();

    // Try to run a basic system utility that's likely present
    let config = AppConfig {
        name: "bundle-test".to_string(),
        command: "/usr/bin/say".to_string(),
        args: vec!["-v".to_string(), "?".to_string()], // List voices
        cwd: PathBuf::from("."),
        ..Default::default()
    };

    let mut handle = supervisor.spawn(&config).await.unwrap();
    let status = timeout(Duration::from_secs(5), supervisor.wait(&mut handle)).await;
...
#[tokio::test]
async fn test_macos_supervisor_unicode_handling() {
    ...
    let config = AppConfig {
        ...
        command: "echo".to_string(),
        args: vec!["你好世界 🌍 emoji test".to_string()],
        cwd: PathBuf::from("."),
        ..Default::default()
    };
...
#[tokio::test]
async fn test_macos_supervisor_file_descriptors() {
    ...
    let config = AppConfig {
        ...
        command: "sh".to_string(),
        args: vec!["-c".to_string(), "lsof -p $$ | wc -l".to_string()],
        cwd: PathBuf::from("."),
        ..Default::default()
    };
